...
export const wsCase = new KUnit();

wsCase.test('Connect', async function () {
    await client.connect();
})

wsCase.test('CallApi normally', async function () {
    // Succ
...
wsCase.test('Inner Error', async function () {
...
wsCase.test('TsrpcError', async function () {
    for (let v of ['Test', 'a/b/c/Test']) {
        assert.deepStrictEqual(await client.callApi(v as any, {
            name: 'TsrpcError'
        }).catch(e => ({
...
wsCase.test('sendMsg', async function () {
    let msg: MsgChat = {
        channel: 123,
        userName: 'fff',
        content: '666',
...
wsCase.test('cancel', async function () {
...
wsCase.test('error', async function () {
...
wsCase.test('client timeout', async function () {
...
wsCase.test('send/listen Msg', async function () {
    let recved: MsgChat[] = [];
...
wsCase.test('disconnect', async function () {
    await client.disconnect();
})